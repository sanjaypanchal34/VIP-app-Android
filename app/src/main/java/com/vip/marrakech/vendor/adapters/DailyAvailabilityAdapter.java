...
public class DailyAvailabilityAdapter extends GRecyclerAdapter<DateAvailabilityModel, DailyAvailabilityAdapter.ViewHolder> {
    public DailyAvailabilityAdapter(Context context, List<DateAvailabilityModel> list) {
        super(context, list);
    }

    @Override
    public ViewHolder setItemRow(ViewGroup viewGroup, int i) {
        return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.item_availability_daily, viewGroup, false));
    }
    ...
    public void myBindViewHolder(final ViewHolder viewHolder, int i, final List<DateAvailabilityModel> mList) {
        ...
        viewHolder.tv_date.setText(model.getId());
